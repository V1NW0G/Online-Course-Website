...
<h2></h2>
 <center>
        <div id="logincard" class="card text-dark bg-light mb-3 text-center" style="max-width: 35rem;" >
                    <div class="card-header text-dark bg-light mb-3"><h2><fmt:message key="language.lecture"/> #${ticket.id}</h2></div>
                        <div class="card-body text-dark bg-light mb-3">
                        <form:form method="POST" enctype="multipart/form-data" modelAttribute="ticketForm">   
                            <form:label path="subject"><fmt:message key="language.subject"/></form:label><br/>
                            ...
                            <form:label path="body"><fmt:message key="language.body"/></form:label><br/>
                            ...
                            <c:if test="${fn:length(ticket.attachments) > 0}">
                                ...
                                <ul>
                                    <c:forEach items="${ticket.attachments}" var="attachment">
                                        ...
                                        <h5><c:out value="${attachment.name}" /></h5><a href="<c:url value="/ticket/delete/${ticket.id}"/>" class="btn btn-danger " style="width: 6rem;"><fmt:message key="language.delete"/><i class="bi bi-trash"></i></a>
                            ...
                            <b><fmt:message key="language.addAtatch"/></b><br />
                            ...
                            <button type="submit" class="btn btn-primary d-grid gap-2 col-8 mx-auto " style="height: 3rem;"><h5><fmt:message key="language.save"/></h5></button><br/>
                        </form:form>
                        <a href="<c:url value="/ticket" />"><fmt:message key="language.returnlist"/></a>
                        </div>
        </div>
</body>
</html> 