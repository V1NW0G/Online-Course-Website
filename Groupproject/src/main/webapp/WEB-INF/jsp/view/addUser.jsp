...
<html lang="${param.lang}">
    ...
    <center>
        <div id="logincard" class="card text-dark bg-light mb-3 text-center" style="max-width: 35rem;" >
                    <div class="card-header text-dark bg-light mb-3"><h2><fmt:message key="language.CreateUser"/></h2></div>
                        <div class="card-body text-dark bg-light mb-3">
                        <form:form method="POST" enctype="multipart/form-data" modelAttribute="ticketUser">
                            <form:label path="username" ><fmt:message key="language.username"/></form:label><br/>
                            <form:input type="text" path="username" placeholder="Username" class="d-grid gap-2 col-8 mx-auto outline-secondary" style="height: 3rem;"  /><br/>
                            <form:label path="password"><fmt:message key="language.password"/></form:label><br/>
                            <form:input type="text" path="password" placeholder="Password" class="d-grid gap-2 col-8 mx-auto outline-secondary" style="height: 3rem;" /><br/>
                            <form:label path="roles"><fmt:message key="language.role"/></form:label><br/>
                            <form:checkbox path="roles" value="ROLE_USER" /><fmt:message key="language.roleUser"/>
                            </br>
                            <form:checkbox path="roles" value="ROLE_ADMIN" /><fmt:message key="language.roleAdmin"/>
                            <br /><br />

                            <button type="submit" class="btn btn-primary d-grid gap-2 col-8 mx-auto " style="height: 3rem;"><h5><fmt:message key="language.addUser"/></h5></button><br/>
                        </form:form>
                        </div>
        </div>
</center>
